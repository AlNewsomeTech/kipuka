...
export default function RichTextField({ label, value, onChange, placeholder, disabled }) {
  return (
    <div>
      ...
      <div className={`rounded-lg border border-slate-200 bg-white overflow-hidden ${disabled ? 'opacity-60 pointer-events-none' : ''}`}>
        <ReactQuill theme="snow" value={value || ''} onChange={onChange} modules={MODULES} placeholder={placeholder} readOnly={disabled} />
      </div>
    </div>
  );
}